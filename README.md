...
> **Lưu ý Windows:** `pyiceberg` hiện chỉ có wheel Windows đến Python 3.13.
> `make setup` tự chọn Python 3.13 hoặc thấp hơn khi dùng `uv`, nên không cần cài Microsoft C++ Build Tools.
...
1. **`VACUUM` không dọn orphan chưa từng commit.** `deltalake` (Rust/Python) chỉ thu hồi file đã bị _tombstone_ trong log. File do job crash để lại chưa từng vào log → vô hình với vacuum ở mọi retention. NB6 đo, rồi bắt bạn tự viết phép hiệu tập hợp.
2. **`expire_snapshots` của Iceberg chỉ đụng metadata.** 20 → 3 snapshot nhưng **0 file avro bị xoá**; metadata còn _phình ra_. Job 3 và Job 4 là một **cặp** — chạy expiry mà không quét orphan là lý do "đã expire mà hoá đơn S3 không giảm".
...
_Plugin 'container-compose' not found_) và **không expose Docker API socket**,
...
3. **NB3** — MERGE 100K + RESTORE; `history()` ≥ 5 version _sau_ restore
...
| Triệu chứng                                                    | Fix                                                                                             |
| -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| `make setup` báo `python3: command not found`                  | Cài Python 3.10–3.13 hoặc `uv`                                                                  |
...
1. **Compose interpolation** — `${f%.py}` trong khối `command:` bị Compose hiểu là
   ...
   đôi dấu `$` (kể cả trong dòng _comment_ — Compose nội suy cả comment).
...
3. **`PYTHONPATH` bị ghi đè** — image này expose `pyspark` _chỉ_ qua `PYTHONPATH`;
...
4. **Ivy cache không ghi được** — named volume gắn ở `~/.ivy2` (đường dẫn _không có_
   ...
   `spark.jars.ivy` vào `~/.cache/ivy` — thư mục _có_ trong image nên volume thừa
   kế quyền của `jovyan`.

Ngoài ra việc convert `.py`→`.ipynb` nay là _best-effort_: trên host Linux mà UID
